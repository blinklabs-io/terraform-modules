...
variable "cluster_name" {
  ...
  validation {
    ...
    error_message = "cluster_name must start and end with a lowercase letter or digit, contain only lowercase letters, digits, and hyphens (max 40 chars)."
  }
}
...
variable "node_service_account" {
  ...
  validation {
    condition     = var.node_service_account == null || length(trimspace(var.node_service_account)) > 0
    error_message = "node_service_account must be null or a non-empty service account email."
  }
}
